...
ggplot(data = sf_emissions_plot, aes(x = calendar_year, y = emissions_sum)) +
  ...
  scale_x_continuous(breaks = c(2000, 2010, 2020),
                     limits = c(2000, 2020),
                     expand = c(0, 0)) +
  labs(x = "Year", y = "CO2 Emissions (metric ton)", title = "CO2 Emissions of Railways Based on Commodity") +
  theme_light() +
  theme(
      title = element_text(face = "bold"),
      axis.title.y = element_text(angle = 360, face = "bold", vjust = 0.6)
    )

```
```{r}
#  labs(x = "CO2 Emissions (metric ton)", y = "Sector Type", title = "CO2 Emissions of Sectors in SF") +
#  theme_light() +
#  theme(
#    axis.title.y = element_text(angle = 360, face = "bold", vjust = 0.6),
#    title = element_text(face = "bold"),
...
# aes(y = fct_reorder(sector_general, emissions_sum), x = emissions_sum))
```
